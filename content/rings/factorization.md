...
Recordemos que la inclusión $R\subset K$ se extiende al cuerpo de fracciones $k$ de $R$, $R\subset k\subset K$. Para probar que $\ker g=(f)$ consideramos la extensión $g'\colon k[ x ]\rightarrow K$ de $g$ que se define como la inclusión $k\subset K$ sobre el cuerpo de coeficientes y que cumple $g'(x)=\alpha$. Veamos que $\ker g'=(f)\subset k[ x ]$.

El ideal $\ker g'\subset k[ x ]$ está formado por todos los polinomios que tienen a $\alpha$ como raíz. Al ser $k[ x ]$ un dominio euclídeo, $\ker g'=(\tilde f)$ donde $\tilde f\in k[ x ]$ es cualquier polinomio no nulo de grado mínimo en este ideal. Realizamos la división euclídea en $k[ x ]$, $f(x)=c(x)\tilde{f}(x)+r(x)$. Si $r$ no fuera trivial, su grado sería menor que el de $\tilde{f}$, pero $r(x)=f(x)-c(x)\tilde{f}(x)$, por tanto $r(\alpha)=f(\alpha)-c(\alpha)\tilde{f}(\alpha)=0-c(\alpha)0=0$. Esto es imposible porque $\tilde{f}$ es de grado mínimo. Por tanto $r=0$ y $f(x)=c(x)\tilde{f}(x)$. El polinomio $c(x)$ ha de ser constante porque $f$ es también irreducible en $k[ x ]$, así que $f$ y $\tilde{f}$ son asociados, luego generan el mismo ideal, $\ker g'=(f)\subset k[ x ]$.

Como $g=g'|_{R[ x ]}$, $\ker g=\ker g'\cap R[ x ]=(f)\subset R[ x ]$ en virtud de la proposición anterior, pues $f\in R[ x ]$, al ser irreducible, es primitivo. Esto concluye la demostración.
{{% /proof %}}
